...
Consulting literature on combinatorial depth measures [@D_F_L_M_S], I learned that instead of directly counting the triangles that do contain the query point (which requires looking at triplets of points), the combinatorial approach counts the triangles that do not contain the query point, and subtracts that from the total $n \choose 3$. Referencing Aloupis's work, [@D_F_L_M_S] states the combinatorial simplicial depth definition of a query point $q$ in $R^2$ as $${n \choose 3} - \sum_{i = 1}^{n} {h_{i}(q) \choose 2}$$ where $h_{i}(q)$ denotes the halfspace counts, i.e. the number of points that lie to the right of the line that passes through $q$ and the $i^{th}$ data point. 
