...
#function to calculate theta - the true angle between positive x-axis and the
#point going counter-clockwise
theta = function(x, y){
  answer = atan2(y,x) 
  if((x < 0 && y < 0) || 
     (x > 0 && y < 0) ||
...
#we also need to calculate the true angle that the shifted point forms with the x-axis
thetaAngle = numeric()
...
#assign each angle the correct reference value
...
ddDepth = depth.halfspace(targetPoint, df) * nrow(df)
#ddalpha outputs
ddDepth